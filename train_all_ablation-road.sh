...
python3 -m torch.distributed.launch --nproc_per_node=$GPUS train.py --config /root/tqdm/configs/tqt/tqt_eva_vit-b_1e-5_5k-r2r-512-$scene_type-traversable-pixel-proj-cls-prefix-224x224-nopromptcls.py --launcher pytorch --gpus $GPUS
wait

python3 -m torch.distributed.launch --nproc_per_node=$GPUS train.py --config /root/tqdm/configs/tqt/tqt_eva_vit-b_1e-5_5k-r2r-512-$scene_type-traversable-pixel-proj-cls-prefix-224x224-nocrossattn.py --launcher pytorch --gpus $GPUS
wait

...
# from models.segmentors import OTFeatureAligner
...
class tqt_EVA_CLIP(tqdm_EVA_CLIP):
    ...
    def __init__(self,
        ...
        if self.use_sne:
            ...
            self.opt_sne = OTFeatureAligner(dim=out_channels)
            self.opt_img = OTFeatureAligner(dim=out_channels)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, **kwargs):
        ...
        if self.use_sne and self.feature_phase == 'pixel' and self.feature_mode == 'proj':
            ...
            x_visual,loss_opt_img, pi_img = self.opt_img(x_visual.view(x_visual.size(0), x_visual.size(1), -1).permute(0,2,1).contiguous(), text_embeddings)
            sne_visual,loss_opt_sne, pi_sne = self.opt_sne(sne_visual.view(sne_visual.size(0), sne_visual.size(1), -1).permute(0,2,1).contiguous(), text_embeddings)
            losses.update(add_prefix({'loss': loss_opt_sne}, 'sne_opt'))
            losses.update(add_prefix({'loss': loss_opt_img}, 'img_opt'))

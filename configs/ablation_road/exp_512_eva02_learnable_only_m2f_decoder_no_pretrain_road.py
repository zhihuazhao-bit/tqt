"""
消融实验: Learnable Only + 标准 Mask2Former Pixel Decoder + 无预训练权重 (Road3D 数据集)
- 尺寸: 512x512
- 权重: 无预训练权重 (from scratch)
- SNE: 无
...
- 不使用任何文本编码器
- 不使用 EVA02-CLIP 预训练权重，从头训练
- 使用标准 Mask2Former pixel decoder (仅 self-attention，无 cross-attention)
- 对比 exp_512_eva02_learnable_only_m2f_decoder_road.py (使用预训练权重)
"""
_base_ = [
...
pretrained_weight = None  # 不使用预训练权重
use_sne = False
...
exp_name = 'ablation_512_eva02_learnable_only_m2f_decoder_no_pretrain_road'
...
model = dict(
    ...
    eva_clip=dict(
        ...
        pretrained=pretrained_weight,  # None = 不使用预训练权重

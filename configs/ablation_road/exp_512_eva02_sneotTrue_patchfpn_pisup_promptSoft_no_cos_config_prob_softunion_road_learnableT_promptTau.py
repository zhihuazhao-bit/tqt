"""
F2pSoft-config (Road3D 512): SNE + OT 融合(config) + Patch-FPN + pi 深监督 + Prompt(Soft) + Learnable T
- 尺寸: 512x512
- 数据集: Road3D
- 权重: EVA02_CLIP_B
- SNE: 有 (backbone-ot)
...
prompt_cls_topk_train = None  # 不使用 TopK
prompt_cls_topk_test = None   # 不使用 TopK
patch_fpn = True
...
exp_name = 'ablation_512_eva02_sneotTrue_patchfpn_pisup_promptSoft_no_cos_config_prob_softunion_road_learnableT_promptTau'

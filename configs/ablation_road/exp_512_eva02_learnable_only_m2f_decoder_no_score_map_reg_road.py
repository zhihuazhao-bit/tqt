"""
消融实验: Learnable Only + 标准 Mask2Former Pixel Decoder + 无 Vision-Language Score Map 正则化 (Road3D 数据集)
- 尺寸: 512x512
- 权重: EVA02_CLIP_B (原始)
- SNE: 无
- Prompt: learnable_only (仅可学习掩码向量，无文本编码)
- Pixel Decoder: MSDeformAttnPixelDecoder (标准 Mask2Former，无文本交叉注意力)
- Score Map Reg: 禁用 (use_score_map_reg=False)
- 用途: 验证 Vision-Language score_map 正则化对模型性能的影响

说明:
- 使用 2 个可学习的类别原型向量 (traversable / non-traversable)
- 不使用任何文本编码器
- 使用标准 Mask2Former pixel decoder (仅 self-attention，无 cross-attention)
- 禁用 Vision-Language score_map 正则化 (scr_map, sne_map 损失)
- 对比 exp_512_eva02_learnable_only_m2f_decoder_road.py (使用 score_map 正则化)
"""
_base_ = [
...
exp_name = 'ablation_512_eva02_learnable_only_m2f_decoder_no_score_map_reg_road'
...
model = dict(
    ...
    use_score_map_reg=use_score_map_reg,  # 禁用 score_map 正则化

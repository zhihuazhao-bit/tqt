"""
F2pSoft-config-topk (Road3D 512): SNE + OT 融合(config) + Patch-FPN + pi 深监督 + Prompt(Soft) + Learnable T + TopK
- 尺寸: 512x512
- 数据集: Road3D
- 权重: EVA02_CLIP_B
- SNE: 有 (backbone-ot)
...
- OT SoftUnion: True
- OT Cost: cos
- OT Fuse: config (entropy-based confidence weighting)
- Patch-FPN: True
- Pi Supervision: True
- Prompt Mode: soft
- OT Temperature: Learnable (ot_score_prior_temperature=None)
- Prompt Temperature: Uses Tau (prompt_cls_temperature_mode='tau')
- Tau: Learnable (tau=None)
- TopK: Train=10, Test=3 (similar to dropout)
"""
_base_ = [
...
ot_score_prior_temperature = None  # Learnable
ot_cost_type = 'cos'
ot_fuse_output = False
ot_fuse_mode = 'config'  # entropy-based confidence weighting
ot_softunion = True
prompt_cls = True
prompt_cls_mode = 'soft'
prompt_cls_temperature_mode = 'tau'  # Use Tau
prompt_cls_topk_train = 10  # Training TopK
prompt_cls_topk_test = 3    # Testing TopK
patch_fpn = True
...
exp_name = 'ablation_512_eva02_sneotTrue_patchfpn_pisup_promptSoft_no_cos_config_prob_softunion_road_learnableT_promptTau_topk'

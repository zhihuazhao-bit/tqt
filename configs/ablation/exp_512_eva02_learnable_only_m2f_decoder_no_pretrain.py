"""
消融实验: Learnable Only + 标准 Mask2Former Pixel Decoder + 可配置预训练权重 (ORFD 数据集)
- 尺寸: 512x512
- 权重: 可通过 load_text_pretrained 和 load_image_pretrained 独立控制
- SNE: 无
- Prompt: learnable_only (仅可学习掩码向量，无文本编码)
- Pixel Decoder: MSDeformAttnPixelDecoder (标准 Mask2Former，无文本交叉注意力)
- 用途: 验证预训练权重对模型性能的影响

预训练权重加载方式 (四种组合):
- load_text_pretrained=True,  load_image_pretrained=True  -> 全部加载
- load_text_pretrained=True,  load_image_pretrained=False -> 只加载 text encoder
- load_text_pretrained=False, load_image_pretrained=True  -> 只加载 image encoder
- load_text_pretrained=False, load_image_pretrained=False -> 全部不加载 (from scratch)

说明:
- 使用 2 个可学习的类别原型向量 (traversable / non-traversable)
- 不使用任何文本编码器
- 使用标准 Mask2Former pixel decoder (仅 self-attention，无 cross-attention)
- 对比 exp_512_eva02_learnable_only_m2f_decoder.py (使用预训练权重)
"""
_base_ = [
...
pretrained_weight = 'weight/pretrained/EVA02_CLIP_B_psz16_s8B.pt'  # 预训练权重路径
# 控制预训练权重加载方式 (四种组合):
# - load_text_pretrained=True,  load_image_pretrained=True  -> 全部加载
# - load_text_pretrained=True,  load_image_pretrained=False -> 只加载 text encoder
# - load_text_pretrained=False, load_image_pretrained=True  -> 只加载 image encoder
# - load_text_pretrained=False, load_image_pretrained=False -> 全部不加载 (from scratch)
load_text_pretrained = False   # 是否加载 text encoder 预训练权重
load_image_pretrained = False  # 是否加载 image encoder 预训练权重
use_sne = False
sne_fusion_stage = 'backbone'
sne_fusion_mode = 'proj'
prompt_cls = True                      # 启用 (必须为 True)
prompt_cls_type = 'learnable_only'     # 仅可学习向量模式
# ============================================================================
...
exp_name = 'ablation_512_eva02_learnable_only_m2f_decoder_no_pretrain'
...
model = dict(
    ...
    # learnable_only 模式会自动禁用以下配置:
    # - use_context_decoder -> False
    # - textual_reg, visual_reg 相关损失会被跳过
    ...
    eva_clip=dict(
        ...
        context_length=32,
        xattn=True),
    # 注意: learnable_only 模式下 context_decoder 会被自动禁用
    ...
    decode_head=dict(
        ...
        # ============================================================
        # 关键改动: 使用标准 Mask2Former Pixel Decoder
        # - 移除文本交叉注意力 (cross_attn)
        # - 使用 DetrTransformerEncoder 替代 DetrTransformerDecoder
        # - operation_order: self_attn -> ffn (无 cross_attn)
        # ============================================================
        pixel_decoder=dict(
    ...
    visual_reg=False,     # learnable_only 模式禁用
    textual_reg=False,    # learnable_only 模式禁用
)
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'learnable_class_prototypes': dict(lr_mult=1.0),

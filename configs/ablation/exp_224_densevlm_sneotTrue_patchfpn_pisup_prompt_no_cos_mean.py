"""
消融实验 F2c+pi (mean, cos): SNE + OT 融合 (有先验, mean 池化) + Patch-FPN + pi 深监督 (EVA02 权重, ORFD)
- 尺寸: 224x224
- 权重: EVA02_CLIP_B (原始)
...
对比实验: F2c+pi(proj, cos) vs F2c+pi(mean, cos)
"""
_base_ = [
...
exp_name = 'ablation_224_densevlm_sneotTrue_patchfpn_pisup_prompt_no_cos_mean'

"""
消融实验 F2d-xsam: SNE + OT 融合(有先验) + Patch-FPN (PixelSampling) + pi 深监督 (EVA02 权重, ORFD)
- 尺寸: 224x224
- 权重: EVA02_CLIP_B (原始)
- SNE: 有 (backbone-ot)
- Prompt: 无
- OT Prior: True
- Patch-FPN: PixelSampling 版本 (patch_fpn_xsam=True)
- Pi Supervision: True

对比实验: F2d (ConvTranspose/Conv 版 Patch-FPN) vs F2d-xsam (PixelShuffle 版 Patch-FPN)
"""
_base_ = [
...
prompt_cls = False
patch_fpn = False
patch_fpn_xsam = True  # PixelSampling 版 Patch-FPN
...
exp_name = 'ablation_224_eva02_sneotTrue_patchfpn_xsam_pisup_noprompt'

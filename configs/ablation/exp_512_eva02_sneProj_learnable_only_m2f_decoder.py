"""
消融实验: Learnable Only + 标准 Mask2Former Pixel Decoder + SNE Proj 融合 (ORFD 数据集)
- 尺寸: 512x512
- 权重: EVA02_CLIP_B (原始)
- SNE: 启用 (proj 融合，backbone 阶段)
- Prompt: learnable_only (仅可学习掩码向量，无文本编码)
- Pixel Decoder: MSDeformAttnPixelDecoder (标准 Mask2Former，无文本交叉注意力)
- 用途: 验证 SNE proj 融合在无文本模态 + 标准 pixel decoder 下的效果

说明:
- 使用 2 个可学习的类别原型向量 (traversable / non-traversable)
- 不使用任何文本编码器
- 使用标准 Mask2Former pixel decoder (仅 self-attention，无 cross-attention)
- 启用 SNE proj 融合，在 backbone 阶段融合表面法线特征
- 对比 exp_512_eva02_learnable_only_m2f_decoder.py (无 SNE)
"""
_base_ = [
...
use_sne = True                          # ✅ 启用 SNE
sne_fusion_stage = 'backbone'           # 在 backbone 阶段融合
sne_fusion_mode = 'proj'                # 使用 proj 融合模式
...
exp_name = 'ablation_512_eva02_sneProj_learnable_only_m2f_decoder'

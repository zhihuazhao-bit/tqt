"""
消融实验 #6: SNE OT + Patch-FPN + PromptSoft + Text Pretrain Only (ORFD)
- 基于: exp_512_eva02_sneotTrue_patchfpn_pisup_promptSoft_no_cos_config_prob_softunion_learnableT_promptTau.py
...
exp_name = 'ablation_512_eva02_sneot_patchfpn_text_pretrain'
...
model = dict(
    ...
    prompt_cls_mode=prompt_cls_mode,
    prompt_cls_temperature_mode=prompt_cls_temperature_mode,
    prompt_cls_topk_train=prompt_cls_topk_train,
    prompt_cls_topk_test=prompt_cls_topk_test,
    use_sne=use_sne,
    ...
    decode_head=dict(
        ...
        pixel_decoder=dict(
            ...
            encoder=dict(
                ...
                transformerlayers=dict(
                    ...
                    attn_cfgs=[
                        dict(type='MultiScaleDeformableAttention', embed_dims=256, num_heads=8, num_levels=3, num_points=4, im2col_step=64, dropout=0.0, batch_first=False),
                        dict(type='MultiheadAttention', embed_dims=256, num_heads=8, attn_drop=0.0, proj_drop=0.0, batch_first=False)
                    ],
                    ffn_cfgs=dict(embed_dims=256, feedforward_channels=visual_feature_dim, num_fcs=2, act_cfg=dict(type='ReLU', inplace=True), ffn_drop=0.0),
        ...
        transformer_decoder=dict(
            ...
            transformerlayers=dict(
                type='DetrTransformerDecoderLayer',
                attn_cfgs=dict(type='MultiheadAttention', embed_dims=256, num_heads=8, attn_drop=0.0, proj_drop=0.0, batch_first=False),
                ffn_cfgs=dict(embed_dims=256, feedforward_channels=visual_feature_dim*2, num_fcs=2, act_cfg=dict(type='ReLU', inplace=True), ffn_drop=0.0),
                feedforward_channels=visual_feature_dim*2,
